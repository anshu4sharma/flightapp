...
const FlightsResult = () => {
  ...
  const checkboxFilter = () => {
    if (airlinesInfo.length < 1) {
      setFlightData(myData)
    } else {
      const filteredDate = myData.filter((data) => {
        return airlinesInfo.includes(data.Airline)
      })
  ...
  const handleChange = (e) => {
    ...
    if (checked) {
      setairlinesInfo([...airlinesInfo, value])
    } else {
      setairlinesInfo(airlinesInfo.filter((e) => e !== value))
    }
  }
  useEffect(() => {
    checkboxFilter()
  ...
  return (
    <section className="text-gray-600 body-font flex sm:flex-row flex-col items-center ">
      <div className="flex w-max sm:w-64 mb-10 flex-col px-14 py-2 shadow-xl">
        <div className="flex items-start flex-col space-x-3 py-6">
          <label
            htmlFor="filterPrice"
            className="text-xl my-4 text-black font-bold"
          >
            Filter Price
          </label>
          <input
        ...
        <div className="-my-4 mx-4 rounded-2xl grid grid-cols-1 sm:grid-cols-2 gap-4 divide-gray-100">
          {flightData
            .filter((data) => {
              if (
                data.country ===
                  `${selectedOrigin} to ${selectedDestination}` &&
            ...
            .map((data, index) => {
              return (
                <div
                  key={index}
                  className="py-4 my-3 flex flex-wrap gap-16 md:gap-0 md:flex-nowrap rounded-3xl px-4 border border-white shadow-2xl"
                >
